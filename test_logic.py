...
@pytest.fixture
def standard_balances():
    return {6: 32, 5: 16, 4: 8, 3: 4, 2: 2, 1: 1}


# Test 1 - multiplication test for one level
# Expected result:
def test_calculate_possessed_sum_basic(standard_balances):
...
def test_calculate_possessed_sum_multiple_levels(standard_balances):
...
def test_calculate_possessed_sum_empty(standard_balances):
    counts = {}
...
def test_calculate_possessed_sum_param(counts, expected, standard_balances):
    assert calculate_possessed_sum(counts, standard_balances) == expected


# Test 5 - Case with unknown level
def test_calculate_possessed_sum_unknown_level(standard_balances):
    counts = {7: 1}
...
def test_get_missing_amount_parametrized(possessed_sum, target, expected):
