...
@pytest.fixture
def levels_full() -> dict[int, int]:
    return {6: 1, 5: 1, 4: 1, 3: 1, 2: 1, 1: 1}


@pytest.fixture
def levels_empty() -> dict[int, int]:
    return {6: 0, 5: 0, 4: 0, 3: 0, 2: 0, 1: 0}


@pytest.fixture
def levels_partial() -> dict[int, int]:
...
def make_dino(levels: dict[int, int], totems: int = 0) -> dict[str, Any]:
    """
...
def test_total_value_full(levels_full: dict[int, int]) -> None:
    assert calculate_total_value(levels_full) == 63


def test_total_value_empty(levels_empty: dict[int, int]) -> None:
    assert calculate_total_value(levels_empty) == 0


def test_total_value_partial(levels_partial: dict[int, int]) -> None:
...
def test_effective_totems_no_bonus(levels_partial: dict[int, int]) -> None:
    assert calculate_effective_totems(0, levels_partial) == 0


def test_effective_totems_gain_one(levels_full: dict[int, int]) -> None:
    assert calculate_effective_totems(0, levels_full) == 1


def test_effective_totems_cap_at_three(levels_full: dict[int, int]) -> None:
...
def test_golden_chest_true_when_3_totems_amd_full_enclosure() -> None:
    levels: dict[int, int] = {6: 1, 5: 1, 4: 1, 3: 1, 2: 1, 1: 1}
    dino: dict[str, Any] = make_dino(levels, totems=3)
...
def test_golden_chest_false_when_3_totems_but_not_full_enclosure() -> None:
    levels: dict[int, int] = {6: 1, 5: 1, 4: 0, 3: 1, 2: 1, 1: 1}
    dino: dict[str, Any] = make_dino(levels, totems=3)
...
def test_golden_chest_false_when_full_enclosure_but_only_1_totem() -> None:
    levels: dict[int, int] = {6: 1, 5: 1, 4: 1, 3: 1, 2: 1, 1: 1}
    dino: dict[str, Any] = make_dino(levels, totems=1)
...
def test_golden_chest_true_when_effective_totems_reach_3_from_value() -> None:
    ...
    levels: dict[int, int] = {6: 6, 5: 1, 4: 1, 3: 1, 2: 1, 1: 1}
    dino: dict[str, Any] = make_dino(levels, totems=0)
...
def test_golden_chest_false_when_eff3_but_not_full() -> None:
    # 6:6 gives 192 values ​​→ 3 totems
    # but no level 4 → incomplete enclosure
    levels: dict[int, int] = {6: 6, 5: 1, 4: 0, 3: 1, 2: 1, 1: 1}
    dino: dict[str, Any] = make_dino(levels, totems=0)
...
def test_progress_basic() -> None:
    ...
    result: dict[str, Any] = calculate_progress(dino)
